...
{
    __m128i tmp[2];
    const __m128i* ptr = (const __m128i *)ctx;

    tmp[0] = _mm_loadu_si128((const __m128i*)ptr);

    tmp[1] = AES_128_key_exp(tmp[0], 0x01);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x02);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x04);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x08);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x10);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x20);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x40);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x80);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x1B);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];

    tmp[1] = AES_128_key_exp(tmp[0], 0x36);
    if (memcmp(tmp + 1, ++ptr, sizeof(__m128i))) return false;

    _mm_storeu_si128((__m128i*)key, *(__m128i*)ctx);
...
{
    if (aes128_detect_enc(ctx, key)/* || aes128_detect_enc<false>(ctx, key)*/)
    {
        return 16;
    }
    /*
...
static bool aes128_detect_decF(const uint32_t* ctx, uint8_t* key)
{
    ...
    tmp[1] = AES_128_key_exp(tmp[0], 0x01);
    tmp[2] = _mm_aesimc_si128(tmp[1]);
    if (memcmp(tmp + 2, ++ptr, sizeof(__m128i))) return false;
    tmp[0] = tmp[1];
...
{
    if (aes128_detect_decF(ctx, key)/* || aes128_detect_decB<reversed>(ctx, key)*/)
    {
        return 16;
    }
    /*
